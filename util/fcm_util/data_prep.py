...
from ..preprocessing.get_multivariate import expand_series, expand_dim_with_lags
...
def import_from_arff(train_path, test_path, class_train=1, class_test=None, dims=1, specificFiles=None, specTestFile=None, min_max_scale=None, rescLimits=None, paa_window=0, expand_dims_list=False):
    if class_test == None:
    ...
    # discretizing all dimensions
    if paa_window > 0:
        train_series_set = get_disc_data(train_series_set, paa_window)
        test_series_set = get_disc_data(test_series_set, paa_window)

    # adding new dimensions based on lags
    if expand_dims_list:
        train_series_set = expand_dim_with_lags(train_series_set, expand_dims_list)
        test_series_set = expand_dim_with_lags(test_series_set, expand_dims_list)
